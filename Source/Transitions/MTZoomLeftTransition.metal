...
float2 zoomCircle1(float2 uv, float amount) {
    if(amount<0.5) {
...
fragment float4 ZoomLeftFragment(VertexOut vertexIn [[ stage_in ]],
                                ...
                                sampler textureSampler [[ sampler(0) ]])
{
    ...
    if(progress<0.5){
        ...
        return mix(getFromColor(zoomCircle1(uv, smoothstep(0.0, nQuick, progress)), fromTexture, ratio, _fromR),
        ...
        float4 f= mix(d, e, step(1.0-p.x,(progress-0.5)*2.0));
        return f;
    }
}
